...
python -m ddbj_gff.repair --gff normalized.gff3 --fasta genome.fa --apply internal-stop-to-misc --out curated.gff3
...
`repair`: `--gff`(`--detect`/`--apply` 時は必須。`--list` のみの場合は不要) `--fasta`（配列を要する操作を選択した場合は必須）
`--transl-table` `--detect` `--json` `--only` `--apply` `--out` `--report`。
