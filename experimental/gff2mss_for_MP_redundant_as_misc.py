...
def create_CDS_feature(mRNA_feature, gene_id, locus_tag, seq, identical_CDSs, as_misc=False):
    ...
    feature_type = "misc_feature" if as_misc else "CDS"
    location = _insdc_location_string(compound_location, len(seq))

    ret.append(["", feature_type, location, "locus_tag", locus_tag])
    if not as_misc:
    ...
    for qualifier, values in mRNA_feature.qualifiers.items():
        ...
        if qualifier in unwanted_qualifiers:
            continue
        if as_misc and qualifier == "product":
            continue
        for value in values:
            ret.append(["", "", "", qualifier, value])
    if as_misc:
        ret.append(["", "", "", "note", f"identical to CDS of {identical_CDSs[0]} from alternative transcript"])
    ret.append(add_note_describing_submitter_id(gene_id, mRNA_feature.id))
    return ret
...
def check_redundant_subfeatures(gene_feature, seq, target="CDS"):
    """
    Check if the subfeatures of the gene feature are redundant.
    target should be CDS or exon
    """
    def _get_location(sub_feature, seq, target):
        if target == "CDS":
            locations = [ssf.location for ssf in sub_feature.sub_features if ssf.type == "CDS"]
        elif target == "exon":
            locations = [ssf.location for ssf in sub_feature.sub_features if ssf.type == "exon"]
        else:
            raise ValueError(f"target should be CDS or exon, but got {target}") 
        if sub_feature.location.strand == 1:
            locations = sorted(locations, key=lambda x: x.start)
        else:
            locations = sorted(locations, key=lambda x: x.start, reverse=True)
        if locations:
            return _insdc_location_string(sum(locations), len(seq))
        else:
            return ""

    dict_redundancy = {}
    sub_features = gene_feature.sub_features
    if all([sf.id.split(".")[-1].isnumeric() for sf in sub_features]):
        sub_features = sorted(sub_features, key=lambda x: int(x.id.split(".")[-1]))
    for sf in sub_features:  # transcript level (mRNA)
        location = _get_location(sf, seq, target)
        dict_redundancy.setdefault(location, []).append(sf.id)
    if "" in dict_redundancy:
        del dict_redundancy[""]
    # for location, transcript_ids in dict_redundancy.items():
    #     if len(transcript_ids) > 1:
    #         sys.stderr.write(f"Redundant {target} features found: {transcript_ids} at {location}\n")
    # print(dict_redundancy)
    return dict_redundancy
...
for record in seq_records:
    ...
    for feature in record.features:  # gene level feature
        if feature.type != "gene":
            continue
        check_redundant_subfeatures(feature, record.seq, target="exon")
        dict_redundancy = check_redundant_subfeatures(feature, record.seq, target="CDS")
        ...
        for sf in feature.sub_features:  # mRNA as well as other RNA level features
            if sf.type == "mRNA":
                list_identical_CDSs = list(dict_redundancy.values())
                # print(sf.id, list_identical_CDSs)
                identical_CDSs = [identical_CDSs for identical_CDSs in list_identical_CDSs if sf.id in identical_CDSs]
                assert len(identical_CDSs) == 1
                identical_CDSs = identical_CDSs[0]
                ret.extend(create_mRNA_feature(sf, gene_id, locus_tag, record.seq))
                if sf.id == identical_CDSs[0]:
                    ret.extend(create_CDS_feature(sf, gene_id, locus_tag, record.seq, identical_CDSs))
                else:
                    ret.extend(create_CDS_feature(sf, gene_id, locus_tag, record.seq, identical_CDSs, as_misc=True))
            else:
