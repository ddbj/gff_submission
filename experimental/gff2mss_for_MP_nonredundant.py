...
if len(sys.argv) > 1 and sys.argv[1] == "tak2":
    ...
    output_ann_file = "SAMD00647144_marchantia_nonredundant.ann"
    ...
    output_ann_file = "SAMD00647143_marchantia_nonredundant.ann"
...
def create_CDS_feature(mRNA_feature, gene_id, locus_tag, seq, identical_CDSs):
    ...
    for qualifier, values in mRNA_feature.qualifiers.items():
        ...
        for value in values:
            ret.append(["", "", "", qualifier, value])
    ret.append(add_note_describing_submitter_id(gene_id, ",".join(identical_CDSs)))
    return ret
...
for record in seq_records:
    ...
    for feature in record.features:  # gene level feature
        ...
        for sf in feature.sub_features:  # output CDS feature
            if sf.type == "mRNA":
                list_identical_CDSs = list(dict_redundancy.values())
                for identical_CDSs in list_identical_CDSs:
